...
async def handle_unknown(message: types.Message):
    if message.text not in ["📖 Все категории 📖", "📄 Наличие\nтоваров 📄", "💡 О магазине", 
                          "👤 Профиль", "📜 Правила 📜", "❤️ Помощь", "🔰 О сервисе"]:
        await message.answer("К сожалению я не смог распознать Вашу команду. Воспользуйтесь кнопками в меню или отправьте /start")
...
# Обработчик для кнопки "Наличие товаров"
@dp.message_handler(lambda m: m.text == "📄 Наличие\nтоваров 📄")
async def availability_handler(message: types.Message):
    text = (
        "<b>➖➖➖🔥Фишинг Ссылка🔥➖➖➖</b>\n"
        "<b>➖➖➖25.01.26 обновление🔥 Фишинг Ссылка🔥➖➖➖</b>\n"
        "                <b>Фишинг</b> | 500 ₽  | ∞\n\n"
        "<b>Фишинг ссылка</b> жирным шрифтом также и <b>Фишинг</b> везде жирным шрифтом в сообщении этом"
    )
    await message.answer(text, parse_mode='HTML')

# Обработчик для кнопки "О магазине"
@dp.message_handler(lambda m: m.text == "💡 О магазине")
async def about_shop_handler(message: types.Message):
    text = "<b>🏠 Магазин:</b> FishingScamming\n<b>⏰ Дата создания:</b> 2026-01-29"
    await message.answer(text, parse_mode='HTML')

# Обработчик для кнопки "Правила"
@dp.message_handler(lambda m: m.text == "📜 Правила 📜")
async def rules_handler(message: types.Message):
    text = "Информация еще не заполнена создателем бота"
    await message.answer(text)

# Обработчик для кнопки "Помощь"
@dp.message_handler(lambda m: m.text == "❤️ Помощь")
async def help_handler(message: types.Message):
    text = "@TORNADO666TORNADO6"
    await message.answer(text)

# Обработчик для кнопки "О сервисе"
@dp.message_handler(lambda m: m.text == "🔰 О сервисе")
async def about_service_handler(message: types.Message):
    text = "Бот создан в сервисе BOT-T"
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.add(InlineKeyboardButton("Создать бота! Жми", url="https://bot-t.com/?ref=4120749"))
    await message.answer(text, reply_markup=keyboard)

# Обработчик для кнопки "Профиль" (оставляем пустым как в исходном коде)
...
async def item_detail(callback_query: types.CallbackQuery):
    ...
    text = (
        ...
        f"Для оплаты A Bank 4323345041141392. В сообщениях перевода ИД ТГ\n"
